...
#def codeCheck(request, code):
    #global totalPrice
    #global counter
    #counter = 1
    #total = request.GET.get('totalValue')
    #code = request.GET.get('code')
    #try:
        #kod_rabatowy = Code.objects.get(code=code)
...
def moreP(request, product_id):
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            ...
            if not created:
                #quantityCart = get_object_or_404(UserItem, product__id=product_id, option=option)
